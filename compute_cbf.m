function compute_cbf(para, asl_timeseries) %TR, MB_factor, tau, PLD, PreLD )
    ...
    delta_M=zeros([size(M,1),size(M,2),size(M,3),size(M,4)-2]);
    indices = 1:2:size(delta_M,4);
    delta_M(:,:,:,indices)=(M(:,:,:,indices+1)-M(:,:,:,indices)/2-M(:,:,:,indices+2)/2);
    indices = 2:2:size(delta_M,4);
    ...
    M0=repmat(mean(M(:,:,:,2:2:end)/(1-exp(-para.TR/T1)),4),[1,1,1,size(delta_M,4)]);
    M0(isnan(M0))=0;
