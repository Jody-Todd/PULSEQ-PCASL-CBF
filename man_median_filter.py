...
def manMedianFilter(patch):
    # Custom function to compute the median of nonzero values.
    # Parameters:
    #   patch: patch of image to compute masked median over
    # Retruns:
    #   median over non-zero values in patch of image
    #
    
    nonzero_values = patch[patch != 0]  # Exclude zero-valued neighbors
    return np.median(nonzero_values) if nonzero_values.size > 0 else 0
...
def filterImg(img_path):
    ...
    for t in range(img_data.shape[-1]):
        filtered_data[..., t] = generic_filter(
            img_data[..., t], 
            function=manMedianFilter, 
            size=(3, 3, 3), 
            mode="constant", 
    ...
    nib.save(filtered_nii, os.path.join(parent_dir, filename + '_manmed3v.nii'))
